...
class OpenAIProvider(AIProvider):
    ...
    def generate(
        ...
        return self._chat(messages, temperature, max_tokens, stream, on_token)  # type: ignore[no-any-return]
    ...
    def chat(self, messages, temperature=0.7, max_tokens=2048) -> str:
        return self._chat(messages, temperature, max_tokens, False, None)  # type: ignore[no-any-return]
    ...
    def generate(
        ...
        try:
            if stream:
                ...
                return "".join(full)
            resp = session.post(
                f"{self.api_base}/v1/messages", headers=self._headers(), json=payload, timeout=60
            )
            if resp.status_code == 200:
                data = resp.json()
